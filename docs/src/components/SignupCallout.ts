...
export const SignupCallout = component(() => {
  const state = reactive({
    phase: 'idle' as 'idle' | 'loading' | 'error' | 'success',
    errorMsg: '',
  })
    ...
    <div class="sa-callout">
      <div class="sa-callout-inner">
        ...
        <p class="sa-callout-desc">
          ...
          and
          <a href="https://drag-and-drop.formkit.com" target="_blank" rel="noopener">Drag and Drop</a>
        ...
        <div class="${() => state.phase === 'success' ? 'sa-callout-form-wrap sa-hidden' : 'sa-callout-form-wrap'}">
          <form class="sa-callout-form" @submit="${submit}">
            ...
            <button type="submit" class="sa-callout-submit" disabled="${() => state.phase === 'loading'}">
              <span class="${() => state.phase === 'loading' ? 'sa-hidden' : ''}">Request Early Access</span>
              <svg class="${() => state.phase === 'loading' ? 'sa-spinner' : 'sa-spinner sa-hidden'}" viewBox="0 0 20 20" width="16" height="16" fill="none"><circle cx="10" cy="10" r="8" stroke="currentColor" stroke-width="2.5" opacity="0.25"></circle><path d="M10 2a8 8 0 0 1 8 8" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"></path></svg>
            </button>
          </form>
          <p class="${() => state.phase === 'error' ? 'sa-callout-error' : 'sa-callout-error sa-hidden'}">${() => state.errorMsg}</p>
        </div>
        <div class="${() => state.phase === 'success' ? 'sa-callout-success' : 'sa-callout-success sa-hidden'}">
...
export function SignupCalloutIsland() {
  return html`<div data-island="signup-callout">${SignupCallout()}</div>`
}
